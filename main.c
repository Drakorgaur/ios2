...
void common_part(arguments* args, sem_t* molecules_created, sem_t* sem_ready, atom_queue* queue, element type, bool* ready)
{
    int id = 0;
    if (type == OXYGEN) {
    ...
    if (type == OXYGEN) {
        if (queue->size == args->expected) {
//            printf("O: ready\n");
            ready[OXYGEN_READY] = true;
        }
    } else if (type == HYDROGEN) {
        if (queue->size == args->expected*2) {
//            printf("H: ready\n");
